...
def gconnect():
    ...
    try:
        ...
        oauth_flow = flow_from_clientsecrets('client_secret_626174227976.json', scope='')
    ...
    user = session.query(User).filter_by(emailAddress=login_session['email']).first()
    if not user:
        user = User(username = login_session['username'], emailAddress = login_session['email'])
        session.add(user)
        session.commit()
...
def gdisconnect():
    ...
    if result['status'] == '200':
        ...
        if result2['error_description'] == "Token expired or revoked":
            ...
            del login_session['picture']
            del login_session['UserId']
            response = make_response(json.dumps('Token is expired, logout anyway...'), 200)
            response.headers['Content-Type'] = 'application/json'
            # return response
            return redirect(url_for('showCategories'))
...
def showItemsJSON():
    ...
    for category in categories:        
        ...
        categoryItems = [{"cat_id" : item.category_id, 
                          "description" : item.description,
                          "id" : item.id,
                          "title": item.title} for item in items if item.category_id == category.id]
...
def showItems(name):
    print("In showItems page")
    categories = session.query(Category).order_by(Category.id)
    category = session.query(Category).filter(func.lower(Category.name)==func.lower(name)).one()
    items = session.query(Item).filter_by(category_id = category.id).order_by(desc(Item.added_on))
    return render_template('showItems.html', categories=categories, category=category, 
...
def showItemDetails(categoryName, itemName):
    print("In showItemDetails page")    
    item = session.query(Item).filter(func.lower(Item.title) == func.lower(itemName)).one()
    return render_template('viewItem.html', item=item, login_session=login_session)
...
def editItem(itemName):
    ...
    user = session.query(User).filter_by(emailAddress=login_session['email']).one_or_none()
    ...
    if (request.method == 'POST'):
        ...
        #Should flash message to tell it is updated....
        return redirect(url_for('showItemDetails', categoryName=item.category.name, 
                        itemName = item.title))
...
def createItem(category_id):
    ...
    if (request.method == 'POST'):
        ...
        itemID = totalItems + 1
        newItem = Item(id=itemID, title=newItemTitle, description=newItemDescription, 
                        added_on=datetime.datetime.utcnow(), category_id=category_id, 
        ...
        print (user.id)
        login_session['UserId'] = user.id
        # print (login_session['addedByUserId'])
        print ("!!! item added log end!!!!!!!")

        #Should flash message to tell it is added....
        return redirect(url_for('showItemDetails', categoryName=newItem.category.name, 
...
def deleteItem(itemID):
    ...
    print("userID: ")
    print(userID )
    print ('deletingItem.added_by_user_id: {0}'.format(deletingItem.added_by_user_id))

    # if (deleteItem is None or deleteItem.added_by_user_id != userID):
    if (deletingItem.added_by_user_id != userID):
        response = make_response(json.dumps('Item not belongs to current user or not found.', 400))
        response.headers['Content-Type'] = 'application/json'
        return response
